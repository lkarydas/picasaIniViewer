    ...
    if ~strcmp('.',file_struct(i).name) && ... 
       ~strcmp('..',file_struct(i).name)
        ...
        if file_struct(i).isdir
            file_list = ...
                dir_tree2file_list(file_struct(i).files, file_list, ext, readEXIF);
        end
    end
end